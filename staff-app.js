...
function viewCompetitionDetails(competitionId) {
    Promise.all([
        fetch(`https://production-production-df45.up.railway.app/competition/${competitionId}`).then(r => r.json()),
        fetch(`https://production-production-df45.up.railway.app/participants/${competitionId}`).then(r => r.json()),
        fetch(`https://production-production-df45.up.railway.app/judges`).then(r => r.json())
    ])
...
function loadParticipants(competitionId = '', status = '') {
    let url = 'https://production-production-df45.up.railway.app/participants';
    if (competitionId) {
        url = `https://production-production-df45.up.railway.app/participants/${competitionId}`;
    }
...
function viewParticipantDetails(id) {
    fetch(`https://production-production-df45.up.railway.app/participant/${id}`)
    .then(response => response.json())
    .then(participant => {
       const statusColor = participant.status === 'done' ? '#28a745' : 
                   participant.status === 'ongoing' ? '#ffc107' : '#dc3545';
...
function updateRegistrationStatus(participantId, currentStatus) {
    ...
    if (confirm(confirmMessage)) {
        fetch(`https://production-production-df45.up.railway.app/update-participant-status/${participantId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
...
function viewJudgeDetails(id) {
    fetch(`https://production-production-df45.up.railway.app/judge/${id}`)
    .then(response => response.json())
    .then(judge => {
        const eventIcon = judge.is_pageant ? '👑' : '🎪';
...
function showReports() {
    ...
    Promise.all([
        fetch('https://production-production-df45.up.railway.app/competitions').then(r => r.json()),
        fetch('https://production-production-df45.up.railway.app/participants').then(r => r.json()),
        fetch('https://production-production-df45.up.railway.app/judges').then(r => r.json())
    ])
...
function loadScoringOverview() {
    ...
    Promise.all([
        ...
        fetch(`https://production-production-df45.up.railway.app/participants/${competitionId}`).then(r => r.json()),
        fetch(`https://production-production-df45.up.railway.app/judges`).then(r => r.json()),
        fetch(`https://production-production-df45.up.railway.app/overall-scores/${competitionId}`).then(r => r.json()).catch(() => [])
    ])
...
function updateParticipantStatus(competitionId) {
    Promise.all([
        fetch(`https://production-production-df45.up.railway.app/participants/${competitionId}`).then(r => r.json()),
        fetch(`https://production-production-df45.up.railway.app/overall-scores/${competitionId}`).then(r => r.json()),
        fetch('https://production-production-df45.up.railway.app/judges').then(r => r.json())
    ])
...
function liveSearchParticipants(query) {
    ...
    fetch('https://production-production-df45.up.railway.app/participants')
    .then(response => response.json())
    .then(participants => {
