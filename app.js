...
function showCreateEventTypeForm() {
    ...
    document.getElementById("createEventTypeForm").onsubmit = function(event) {
        ...
        fetch('https://mseufci-judgingsystem.up.railway.app/create-event-type', {
            method: 'POST',
            headers: {
...
function showCriteriaTemplates() {
    ...
    fetch('https://mseufci-judgingsystem.up.railway.app/criteria-templates')
    .then(response => response.json())
    .then(templates => {
...
function showCreateCompetitionForm() {
    ...
    document.getElementById("createCompetitionForm").onsubmit = function(event) {
        ...
        fetch('https://mseufci-judgingsystem.up.railway.app/create-competition', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
function saveCriteria() {
    ...
    fetch('https://mseufci-judgingsystem.up.railway.app/save-competition-criteria', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
function loadParticipants(competitionId = '') {
    const url = competitionId ? 
        `https://mseufci-judgingsystem.up.railway.app/participants/${competitionId}` : 
        'https://mseufci-judgingsystem.up.railway.app/participants';
...
function showAddJudgeForm() {
    ...
    document.getElementById("addJudgeForm").onsubmit = function(event) {
        ...
        fetch('https://mseufci-judgingsystem.up.railway.app/add-judge', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
function setupFlexiblePageant(competitionId, competitionName) {
    ...
    document.getElementById('pageantSetupForm').onsubmit = function(event) {
        ...
        // Send to server
        fetch('https://mseufci-judgingsystem.up.railway.app/create-flexible-pageant', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
function deletePageantSegment(segmentId, competitionId, competitionName) {
    ...
    fetch(`https://mseufci-judgingsystem.up.railway.app/delete-pageant-segment/${segmentId}`, {
        method: 'DELETE'
    })
    .then(response => response.json())
    .then(data => {
        ...
        document.getElementById("content").innerHTML = `
            ...
            <form id="editParticipantForm" style="max-width: 700px;">
                ...
                <div class="form-section">
                    ...
                    <label for="performance_title">Performance/Entry Title:</label>
                    <input type="text" id="performance_title" name="performance_title" value="${participant.performance_title || ''}">
                    
                    <label for="performance_description">Performance Description:</label>
                    <textarea id="performance_description" name="performance_description" rows="3">${participant.performance_description || ''}</textarea>
                </div>
                ...
                <div class="form-section">
                    ...
                    <small style="color: #666; display: block; margin-top: 5px;">
                        Upload your photo to a service like Imgur, PostImages, or use Google Drive direct link
                    </small>
                    
                    ${participant.photo_url ? `
                        <div style="margin: 15px 0; text-align: center;">
                            <p><strong>Current Photo:</strong></p>
                            <img src="${participant.photo_url}" alt="Current photo" style="max-width: 200px; max-height: 250px; border: 2px solid #ddd; border-radius: 8px; margin-top: 10px;"
                                 onerror="this.src='https://via.placeholder.com/200x250/800020/ffffff?text=Photo+Not+Available'; this.style.opacity='0.6';">
                        </div>
                    ` : ''}
                    
                    <label for="talents">Special Talents & Skills:</label>
                    <textarea id="talents" name="talents" rows="3">${participant.talents || ''}</textarea>
                    
                    <label for="special_awards">Awards & Achievements:</label>
                    <textarea id="special_awards" name="special_awards" rows="3">${participant.special_awards || ''}</textarea>
                </div>
                
                <div style="margin-top: 30px;">
                    <input type="submit" value="Update Participant" style="padding: 15px 30px; font-size: 16px;">
                    <button type="button" onclick="showViewParticipants()" class="secondary" style="margin-left: 10px;">Cancel</button>
                </div>
            </form>
        `;
...
function deleteParticipant(id) {
    if (confirm('Are you sure you want to delete this participant?')) {
        fetch(`https://mseufci-judgingsystem.up.railway.app/delete-participant/${id}`, {
            method: 'DELETE',
        })
        .then(response => response.json())
        .then(data => {
...
function deleteJudge(id) {
    if (confirm('Are you sure you want to delete this judge? This will also remove their user account.')) {
        fetch(`https://mseufci-judgingsystem.up.railway.app/delete-judge/${id}`, {
            method: 'DELETE',
        })
        .then(response => response.json())
        .then(data => {
...
function deleteCompetition(id) {
    if (confirm('Are you sure you want to delete this competition? This will also delete all participants, judges, and scores associated with it.')) {
        fetch(`https://mseufci-judgingsystem.up.railway.app/delete-competition/${id}`, {
            method: 'DELETE',
        })
        .then(response => response.json())
        .then(data => {
...
function manageSegmentCriteria(segmentId, segmentName, competitionId) {
    ...
    Promise.all([
        fetch(`https://mseufci-judgingsystem.up.railway.app/competition-criteria/${competitionId}`).then(r => r.json()),
        fetch(`https://mseufci-judgingsystem.up.railway.app/segment-criteria/${segmentId}`).then(r => r.json()).catch(() => [])
    ])
    ...
    document.getElementById("segmentCriteriaForm").onsubmit = function(event) {
        ...
        fetch('https://mseufci-judgingsystem.up.railway.app/assign-segment-criteria', {
            method: 'POST',
